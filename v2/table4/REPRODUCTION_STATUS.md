...
Generated by `eval_table4_official.py`, which is not present in this
repository and targeted different (in-language, non-cross-lingual-only)
annotation pools than the current evaluation. It does not regenerate the
current, authoritative Table 4 — for that, all four languages' intrinsic
evaluation relies on the annotated morpheme test set at
...
see [`table4_final.csv`](table4_final.csv) and
[`Intrinsic_report.md`](Intrinsic_report.md).
