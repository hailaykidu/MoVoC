...
\author{Hailay Teklehaymanot \and Besnik Fazlija \and Wolfgang Nejdl}
\date{}
...
on English$\rightarrow$X translation, MoVoC-Tok produces sharper subword
distributions than BPE in three of four languages and attains the highest
morpheme boundary precision in Tigre. We release the annotations, vocabularies
...
\begin{table}[t]
  ...
  \caption{Morpheme-annotated evaluation sets and MorphScore for MoVoC-Tok.
           MorphScore is the micro-averaged recall of gold morpheme boundaries,
           excluding words the tokenizer left unsegmented. Higher is better.}
  \label{tab:datasets}
\end{table}
...
morphological annotation available for each.
...
\begin{table}[t]
  ...
  \caption{Morpheme boundary precision and normalized R\'enyi entropy
           ($\alpha = 2$) for 32k vocabularies. Precision uses exact
           character-offset matching against gold morpheme boundaries; entropy
           is normalized to $[0, 1]$, lower being sharper. For Tigre and Ge'ez
           the Tigrinya MoVoC-Tok is applied cross-lingually.}
  \label{tab:intrinsic}
\end{table}

Table~\ref{tab:intrinsic} reports boundary precision and R\'enyi entropy for
MoVoC-Tok and BPE at a 32k vocabulary.

\paragraph{Entropy.}
MoVoC-Tok attains lower normalized R\'enyi entropy than BPE in three of four
languages: Amharic (0.62 against 0.66), Tigrinya (0.92 against 0.93) and Tigre
(0.71 against 0.73). Lower entropy indicates a more concentrated token
distribution: constraining merges to morpheme boundaries yields units that
recur more consistently across the corpus, rather than a long tail of
near-duplicate fragments differing only in where a morpheme was cut.

\paragraph{Precision.}
MoVoC-Tok attains the highest boundary precision in Tigre, 63.3 against 60.0
for BPE. Tigre is also the language whose gold annotations are entirely
surface-concatenative, so morpheme boundaries coincide exactly with character
offsets and the boundary projection introduces no approximation.

Across both tokenizers, precision is markedly higher for Tigre (60.0--63.3)
than for the more fusional languages (24.0--36.8). This reflects how
morphological structure maps onto the abugida orthography and affects both
tokenizers equally.
...
BPE leads on both supervised directions in BLEU and chrF++. MoVoC-Tok ranks
second on both directions and is ahead of WordPiece by a wide margin, in BLEU
0.7907 against 0.0534 for English$\rightarrow$Amharic and 0.2710 against 0.0439
for English$\rightarrow$Tigrinya.
...
\paragraph{Morphology-aware segmentation yields sharper subword distributions.}
This is the clearest and most consistent intrinsic result: MoVoC-Tok attains
lower normalized R\'enyi entropy than BPE in three of four languages. The
constraint that merges respect morpheme boundaries produces a vocabulary whose
units recur more consistently, which is exactly what entropy measures.
...
higher precision for Tigre across both tokenizers indicates that a substantial
part of the difficulty in the other languages arises from the mapping between
morphology and abugida orthography rather than from either tokenizer.

\paragraph{Intrinsic and extrinsic behaviour differ.}
MoVoC-Tok leads on intrinsic entropy while BPE leads on downstream BLEU and
chrF++ in the supervised directions. Segmentation quality as measured by
morpheme alignment does not translate directly into translation quality at this
training scale. We report both rather than treating either as a proxy for the
other.
...
BLEU values are low in absolute terms for all three tokenizers, reflecting the
training scale and the difficulty of translating into morphologically rich,
low-resource targets. The comparison is between tokenizers under identical
conditions, not against state-of-the-art translation systems.
...
yields lower R\'enyi entropy than BPE in three of four languages and the
highest morpheme boundary precision in Tigre, showing that constraining subword
merges to morpheme boundaries produces more consistent subword inventories. In
downstream translation at this training scale, BPE remains ahead, with
MoVoC-Tok second and well ahead of WordPiece.
