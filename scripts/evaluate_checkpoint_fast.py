#!/usr/bin/env python3
"""Fast evaluation of a trained checkpoint on a subset of validation set.

Usage:
    python3 scripts/evaluate_checkpoint_fast.py --language_pair en_am --tokenizer movoc_tok --seed 44 --max_examples 10000
"""
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--max_examples", type=int, default=10000, help="Max validation examples to evaluate")
    p.add_argument("--num_beams", type=int, default=1, help="Number of beams for beam search")
    return p.parse_args()
...
def main() -> int:
    ...
    # Load full validation set but limit to max_examples
    val_examples = load_split(language_pair, "validation", REPO_ROOT)
    total_val_size = len(val_examples)
    val_examples = val_examples[:max_examples]
    print(f"Validation examples: {len(val_examples)} (out of {total_val_size} total)")
    ...
    validation_results = {
        ...
        "seed": seed,
        "split": "validation",
        "subset_note": f"evaluated on {len(val_examples)}/{total_val_size} validation examples",
        "num_beams": num_beams,
        **eval_result.to_dict(),
    ...
    status = {
        ...
        "run_dir": str(run_dir),
        "num_beams": num_beams,
        "eval_note": f"subset of {len(val_examples)}/{total_val_size} examples",
    }
