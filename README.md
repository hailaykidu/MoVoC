...
referenced by the paper existed before publication and accompanied the work
described there.

The current state of this repository is a **reconstruction and restoration
of the implementation and experimental pipeline**, begun in July 2026. The
exact historical repository state, and the original experiment artifacts —
trained checkpoints, generated predictions, and the scoring pipeline behind
the reported results — are **not fully recoverable**.

What this repository provides is the methodology described in the paper,
preserved as published, together with the implementation and evaluation
procedures that are available. Published results are reported as the
paper's own values and are kept separate from anything generated by the
reconstructed pipeline; see [Results](#results) and
[`docs/REPRODUCIBILITY.md`](docs/REPRODUCIBILITY.md), which records the
artifact status in detail.
