...
## Repository status note (July 2026)

The MoVoC paper was published in Findings of EMNLP 2025. The repository
referenced by the paper existed before publication.

The current state of this repository is a later reconstruction and
reorganization of the project history, begun in July 2026. Some original
experimental artifacts from the publication period — trained checkpoints,
generated predictions, and the scoring pipeline behind the reported
results — are not available in the current repository.

This repository preserves the published methodology and provides the
available implementation and reproducibility information. Published results
are reported as the paper's own values, kept separate from anything
generated by the reconstructed pipeline; see [Results](#results) and
[`docs/REPRODUCIBILITY.md`](docs/REPRODUCIBILITY.md).

### Reconstruction status (2026-07-31)

MT multi-seed reconstruction completed: 18 runs, 2 languages × 3 tokenizers
× 3 seeds. The published training configuration and experimental design were
reconstructed faithfully, and the pipeline runs end to end — all 18 runs
trained and decoded without error.

The reconstructed models did not reach a converged translation regime, so
this rerun cannot rank the tokenizers, and it supports no claim about
MoVoC-Tok's translation performance in either direction. Numerical
differences from the published Table 3 are expected: the original
checkpoints, logs and scoring pipeline were not recovered. These are
**Version 2 verification results**, not a replacement for or a reproduction
of the published values.

See [`docs/MT_Reconstruction_Audit.md`](docs/MT_Reconstruction_Audit.md) for
details. Intrinsic evaluation (Tables 2 and 4) uses a separate pipeline and
is unaffected.
...
This repository contains **two kinds of numbers**, and they are not
comparable. Before reading any figure below, note which kind it is.

| | Original paper results | Version 2 reconstruction results |
|---|---|---|
| **What** | Values reported in the publication | New measurements from the current code |
| **Where** | [§1 below](#1-published-movoc-paper-results); arXiv:[2509.08812](https://arxiv.org/abs/2509.08812) | `evaluation/results/`, `experiments/multiseed/results/`; archived under [`results/reconstruction_v2/`](results/reconstruction_v2/) |
| **Artifacts** | Checkpoints, predictions and scoring pipeline **not recovered** | Newly trained checkpoints and a reconstructed evaluation pipeline |
| **Status** | Historical publication record | Internal validation evidence; **may differ from the published values** |
| **Data file** | **None** — see [`results/original_paper/`](results/original_paper/) | JSON, with provenance recorded in each file |

**The two must not be mixed.** They must never appear in the same table, and
a reconstructed value must never be described as reproducing, confirming or
contradicting a published one. The scoring pipeline behind the published
Table 3 is unavailable, the scale of its BLEU column is unresolved, and the
MT reconstruction did not reach a converged regime — so the comparison is
not available in either direction.

Full rationale:
[`docs/reconstruction_vs_original.md`](docs/reconstruction_vs_original.md).

The published values are treated as **citations, not data**: never edited,
never recomputed, never replaced. No `table3_original.json` exists, by
design — a hand-transcribed file would be indistinguishable from recovered
experiment output.
...
### 2. Reconstruction of the evaluation pipeline

**No reconstructed measurements are published in this README.**

The original experimental artifacts are not fully available. The current
repository provides a reconstruction of the methodology and evaluation
